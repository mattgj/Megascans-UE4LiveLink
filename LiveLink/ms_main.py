...
def ms_save_master_material():
    ...
    try:
        ...
        if len(mat_) >= 1 and mat_[0].is_a(Material) and 'begin' in clip_content.lower():
            ...
            file_ = open((os.path.join(ms_return_path(), 'megascans', 'materials', material_.get_name()) + '.json'), "w+")
            file_.write(Export_)
            file_.close()

            file_ = open((os.path.join(ms_return_path(), 'megascans', 'materials', material_.get_name()) + '.uemat'), "w+")
